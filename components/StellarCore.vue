...
// The "Stellar core" WebGL stage.
//
// Geometry comes from utils/stellarCore (pure math); this component only turns
// those number arrays into three.js objects and animates them. three.js is
// dynamically imported inside onMounted so it never enters the server bundle or
// blocks hydration.
//
// The whole cluster is three Points objects — three draw calls, whatever the
// grain count.
import type * as THREE_NS from 'three'
import type { FollowConfig, OrbitConfig, ParticleLayer, SequenceConfig } from '~/utils/stellarCore'
...
/** Elapsed *motion* time. Runs 1:1 through playback, decays to a standstill
 *  across the brake, and holds there.
 *
 *  Every rate in the piece — the cluster's spin, each stratum's own spin, the
 *  surge — reads this clock instead of the wall clock, which is what brings them
 *  all to rest together instead of each on its own schedule. The brake is the
 *  integral of a `(1-u)^2` rate profile: rate is exactly 1 where playback ends,
 *  so there is no jolt into the brake, and exactly 0 at the end of it. */
function motionClock(elapsed: number, seq: SequenceConfig) {
    if (elapsed <= seq.playMs) return elapsed
    const u = Math.min(1, (elapsed - seq.playMs) / seq.brakeMs)
    return seq.playMs + (seq.brakeMs * (1 - (1 - u) ** 3)) / 3
}

/** 0 while the cloud is intact, 1 once it has vanished into the centre. Eased in
 *  hard: the rush starts from the standstill, so it has to accelerate out of
 *  nothing rather than open at speed. */
function collapseAmount(elapsed: number, seq: SequenceConfig) {
    const start = seq.playMs + seq.brakeMs + seq.holdMs
    if (elapsed <= start) return 0
    return Math.min(1, (elapsed - start) / seq.collapseMs) ** 2.6
}
...
function onResize() {
    ...
    requestAnimationFrame(() => {
        resizeTicking = false
        sizeStage()
        if (reducedMotion) drawFrame(STELLAR_CORE.sequence.playMs * 0.5)
    })
}

function drawFrame(elapsed: number) {
    if (!THREE || !renderer || !scene || !camera || !group || !axis) return
    const { sequence, orbit, loopMs, loopsPerTurn } = STELLAR_CORE

    // Two clocks. Rotation reads one that brakes to a stop; the collapse runs on
    // wall time, since it has to keep moving after everything else has stopped.
    const motion = motionClock(elapsed, sequence)
    const collapse = collapseAmount(elapsed, sequence)

    const loop = (motion % loopMs) / loopMs
    const angle = (motion / (loopMs * loopsPerTurn)) * Math.PI * 2
...
function stop() {
    ...
    rafId = 0
    // The piece plays once and ends collapsed, so leaving the clock running
    // while off-screen would bring the viewer back to an empty stage. Rewinding
    // on the way out means it replays whenever it is scrolled to.
    startTime = 0
}
...
onMounted(async () => {
    ...
    if (reducedMotion) {
        // No motion: one frame from the middle of playback. The sequence's own
        // end state is an empty stage, which is not a still worth showing.
        drawFrame(STELLAR_CORE.sequence.playMs * 0.5)
        return
    }
